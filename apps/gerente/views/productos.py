...
def producto_api_view(request):
    if request.method == 'GET':
        ...
        categoriaId = request.query_params.get('idCat')
        if categoria != None:
            print(categoria)
            producto = models.Productos.objects.filter(
                categoria__nombreCategoria=categoria)
            if producto.exists() == False:
                data["error"] = "Producto no existe"
                return Response(data=data, status=status.HTTP_400_BAD_REQUEST)
        elif categoriaId != None:
            producto = models.Productos.objects.filter(
                categoria__id=categoriaId)
            if producto.exists() == False:
                data["error"] = "Producto no existe"
                return Response(data=data, status=status.HTTP_400_BAD_REQUEST)
        else:
            producto = models.Productos.objects.all()[:10]
        producto_serializer = serializers.ProductosSerializer(
            producto, many=True)
